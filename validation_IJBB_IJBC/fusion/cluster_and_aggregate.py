    ...
    if features.shape[1] > max_element:
        ...
        with torch.no_grad():
            for split_idx, split in enumerate(splits):
                ...
                fused = infer_with(
                    aggregator, subfeature, subinterm, return_weights=False
                )
...
def aggregator_fuse_ijbb(
    features, template_indexes, intermediates, aggregator, max_feature_num=8
):
    ...
    aggregated = aggregator_fn(
        aggregator, tensor_feature, intermediates, max_feature_num, shuffle=False
    )
    return aggregated
