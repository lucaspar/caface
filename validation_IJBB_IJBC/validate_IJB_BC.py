...
def infer_images(
    model, aggregator, img_root, landmark_list_path, batch_size, use_flip_test
):
    ...
    dataloader = prepare_dataloader(
        img_paths, landmarks, batch_size, num_workers=0, image_size=(112, 112)
    )
    ...
    with torch.no_grad():
        for images, idx in tqdm(dataloader):
            ...
            if use_flip_test:
                ...
                if hyper_param.intermediate_type == "map":
                    flipped_feature, flipped_norm, flipped_intermediate = model(
                        fliped_images.to("cuda:0"), return_intermediate=True
                    )
                    flipped_compressed_intermediate = aggregator.compress_intermediate(
                        intermediate
                    )
                elif hyper_param.intermediate_type == "style":
                    flipped_feature, flipped_norm, flipped_intermediate = model(
                        fliped_images.to("cuda:0"),
                        return_style=[
                            int(i) for i in hyper_param.style_index.split(",")
                        ],
                    )
                    flipped_compressed_intermediate = aggregator.compress_intermediate(
                        intermediate
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ijb_meta_path", type=str, default="IJB/insightface_helper/ijb"
    )
    ...
    parser.add_argument(
        ...
        default="IJBB",
        type=str,
        help="dataset_name, set to IJBC or IJBB",
    )
    parser.add_argument(
        "--fusion_method",
        type=str,
        default="cluster_and_aggregate",
        choices=("average", "cluster_and_aggregate"),
    )
    ...
    save_path = os.path.join(
        f"./{dataset_name}_result", run_name, f"fusion_{args.fusion_method}"
    )
    ...
    templates, medias = eval_helper_fusion.read_template_media_list(
        os.path.join(
            ...
            args.ijb_meta_path,
            f"{dataset_name}/meta",
            f"{dataset_name.lower()}_face_tid_mid.txt",
        )
    )
    p1, p2, label = eval_helper_fusion.read_template_pair_list(
        os.path.join(
            ...
            args.ijb_meta_path,
            f"{dataset_name}/meta",
            f"{dataset_name.lower()}_template_pair_label.txt",
        )
    )

    # # Step 2: Get Image Features
    img_root = os.path.join(
        args.data_root, args.ijb_meta_path, f"{dataset_name}/loose_crop"
    )
    ...
    img_feats, faceness_scores, norms, cinterms = infer_images(
        model=model,
    ...
    for N in Ns:
        ...
        if args.fusion_method == "average":
            ...
            fuse_fn = partial(
                cluster_and_aggregate.aggregator_fuse_ijbb,
                aggregator=aggregator,
                max_feature_num=N,
            )
        else:
        ...
        with torch.no_grad():
            template_norm_feats, unique_templates = (
                eval_helper_fusion.image2template_feature_custom(
                    img_feats,
                    templates,
                    medias,
                    fuse_fn,
        ...
        score = eval_helper_fusion.verification(
            template_norm_feats, unique_templates, p1, p2
        )
        verification_result_str, result_dict = eval_helper_fusion.calc_tpr_fpr(
            score, label
        )
