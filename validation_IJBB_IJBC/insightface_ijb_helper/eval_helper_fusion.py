...
def infer_with_aggregator(aggregator, features, interms):
    ...
    input_feature = torch.tensor(features).squeeze(1).unsqueeze(0).cuda()
    input_interm = torch.tensor(interms).squeeze(1).unsqueeze(0).cuda()
    ...
    with torch.no_grad():
        if input_interm.shape[-1] == 25088:
            # prevent gpu memoery
            T = input_interm.shape[1]
            num_bins = int(np.ceil(T / 300))
    ...
    for count_template, uqt in enumerate(unique_templates):
        ...
        for u, ct in zip(unique_medias, unique_media_counts):
            ...
            if ct == 1:
                ...
                #     media_norm_feats += [np.concatenate([face_norm_feats[ind_m][:,:,0], face_norm_feats[ind_m][:,:,1]], axis=0)]
                #     media_interms += [np.concatenate([interms[ind_m][:,:,0], interms[ind_m][:,:,1]], axis=0)]
                # else:
                media_norm_feats += [face_norm_feats[ind_m]]
                media_interms += [interms[ind_m]]
            else:  # image features from the same video will be aggregated into one feature
                # if fusion_method == 'concat':
                #     if face_norm_feats[ind_m].ndim == 3:
                #         media_norm_feats += [np.concatenate([face_norm_feats[ind_m][:,:,0], face_norm_feats[ind_m][:,:,1]], axis=0)]
                #         media_interms += [np.concatenate([interms[ind_m][:,:,0],interms[ind_m][:,:,1]], axis=0)]
                #     else:
                #         media_norm_feats += [face_norm_feats[ind_m]]
        ...
        # if fusion_method == 'concat':
        #     media_norm_feats = np.expand_dims(np.concatenate(media_norm_feats, axis=0), axis=1)
        #     media_interms = np.expand_dims(np.concatenate(media_interms, axis=0), axis=1)
        # else:
...
def image2template_feature_custom(
    ...
    for count_template, uqt in enumerate(unique_templates):
        ...
        if force_avg_gallery and count_template < 1845:
            can_select_best = False
        else:
            can_select_best = True

        if best_fn is not None and can_select_best:
            best_feature = best_fn(face_norm_feats, ind_t, intermediates)
...
def index2template(img_feats=None, templates=None, medias=None):
    # ==========================================================
    # 1. face image feature l2 normalization. img_feats:[number_image x feats_dim]
    # 2. compute media feature.
    # 3. compute template feature.
    ...
    for count_template, uqt in enumerate(unique_templates):
        ...
        for u, ct in zip(unique_medias, unique_media_counts):
            (ind_m,) = np.where(face_medias == u)
            media_norm_feats.extend(face_norm_feats[ind_m, 0].tolist())
        media_norm_feats = np.array(media_norm_feats)
...
def calc_tpr_fpr(score, label):
    ...
    return tpr_fpr_table, dict(zip(header, tpr_fpr_row_num))
