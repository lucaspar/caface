...
class FaceDetector:
    ...
    def pad_bbox(self, bbox, padding_ratio, image_shape):
        ...
        return (
            max(xmin, 0),
            max(ymin, 0),
            min(image_shape[1], xmax),
            min(image_shape[0], ymax),
        )
    ...
    def detect(self, image) -> Image.Image:
        ...
        (preds,) = self.model.predict(
            rgb_img_np_resized, det_threshold=0.8, iou_threshold=0.4
        )
        ...
        if preds["boxes"]:
            ...
            # pad bbox
            padded_bbox = self.pad_bbox(
                rescaled_bbox, padding_ratio=0.1, image_shape=rgb_img_np.shape
            )
