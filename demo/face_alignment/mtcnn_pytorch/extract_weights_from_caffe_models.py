    ...
    for p in net.params:
        if "conv" in p:
            ...
            if "-" in p:
                ...
                all_weights[name + ".weight"] = net.params[p][0].data.transpose(
                    (0, 1, 3, 2)
                )
...
# P-Net
net = caffe.Net(
    "caffe_models/det1.prototxt", "caffe_models/det1.caffemodel", caffe.TEST
)
np.save("src/weights/pnet.npy", get_all_weights(net))

# R-Net
net = caffe.Net(
    "caffe_models/det2.prototxt", "caffe_models/det2.caffemodel", caffe.TEST
)
np.save("src/weights/rnet.npy", get_all_weights(net))

# O-Net
net = caffe.Net(
    "caffe_models/det3.prototxt", "caffe_models/det3.caffemodel", caffe.TEST
)
