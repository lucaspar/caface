...
class Trainer(trainer_base.BaseTrainer):
    def __init__(self, **kwargs):
        ...
        fusion_model = fusion_net.build_fusion_net(
            model_name=self.hparams.decoder_name, hparams=self.hparams
        )
        ...
        self.aggregator = aggregator.build_aggregator(
            model_name=self.hparams.aggregator_name,
        ...
        self.fuse_fn = partial(
            fusion.aggregator_fuse_ijbb,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hparams.lr_scheduler == "step":
            ...
            lr = self.trainer.lr_schedulers[0]["scheduler"]._get_lr(
                self.trainer.global_step
            )[-1]
        else:
            raise ValueError("not implemetned yet")

        recog_info, recon_info = self.forward(
            images, labels, extra_images, batch_idx, dummy_image, aug_image
        )
        ...
        if self.hparams.center_loss_lambda > 0:
            ...
            for agg_key in agg_keys:
                ...
                for agg in aggregated:
                    cos_loss = cos_loss + (
                        self.cossim_loss(agg, center, detach_y=True)
                        / len(aggregated)
                        / len(agg_keys)
                    )
            loss = loss + (cos_loss * self.hparams.center_loss_lambda)
            self.log(
                "train/cos_loss", cos_loss, on_step=True, on_epoch=True, logger=True
            )
        ...
        if self.hparams.memory_loss_lambda > 0.0:
            ...
            mem_loss = self.cossim_loss(
                memory_aggregated, single_aggregated, detach_y=False
            ) / len(single_aggregated)
            loss = loss + (mem_loss * self.hparams.memory_loss_lambda)
            self.log(
                "train/memory_loss_lambda",
                mem_loss,
                on_step=True,
                on_epoch=True,
                logger=True,
    ...
    def forward(
        ...
        folded_mag_embeddings, folded_intermediate = self.feature_extract(
            clean_images, extra_images
        )

        # split inference in aggregation into two
        if self.hparams.datafeed_scheme == "single":
            agg_result_1 = self.aggregator(
                folded_features=folded_mag_embeddings, intermediate=folded_intermediate
            )
            agg_result_2 = {}
            ...
            for i in list(range(2, half_T + 1)):
                ...
                sub_agg_result_1 = self.aggregator(
                    folded_features=split_mag_emb[0][:, subset_index, :],
                    intermediate=split_intermediate[0][:, subset_index, :],
                )
                sub_agg_result_2 = self.aggregator(
                    folded_features=split_mag_emb[1][:, subset_index, :],
            ...
            if self.hparams.memory_loss_lambda > 0.0:
                ...
                _agg_result_1 = self.aggregator(
                    folded_features=split_mag_emb[0], intermediate=split_intermediate[0]
                )
                _agg_result_2 = self.aggregator(
                    folded_features=split_mag_emb[1], intermediate=split_intermediate[1]
                )
                self.aggregator.memory_count = -1
                self.aggregator.memory = None
                _agg_result_3 = self.aggregator(
                    folded_features=folded_mag_embeddings,
                    intermediate=folded_intermediate,
                )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.hparams.intermediate_type == "map":
            embeddings, norms, intermediate = self.model(
                images, return_intermediate=True
            )
        elif self.hparams.intermediate_type == "style":
            embeddings, norms, intermediate = self.model(
                images, return_style=self.hparams.style_index
            )
        else:
    ...
    def eval_ijbb_set(self, outputs, stage_name):
        ...
        if self.trainer.is_global_zero:
            ...
            ijbb_result_dict = ijb_dataset.ijbb_evaluation(
                data_root,
                ijb_root,
                dataset_name,
                image_features,
            ...
            end_time = time.time()
            print(
                "IJBB evaluation taken time: {} seconds".format(end_time - start_time)
            )
        ...
        if ijbb_result_dict:
            self.log(
                f"ijbb_{stage_name}/0.0001",
                ijbb_result_dict["0.0001"],
                rank_zero_only=True,
            )
            self.log(
                f"ijbb_{stage_name}/0.001",
                ijbb_result_dict["0.001"],
                rank_zero_only=True,
            )
            self.log(
                f"ijbb_{stage_name}/1e-05",
                ijbb_result_dict["1e-05"],
                rank_zero_only=True,
            )
