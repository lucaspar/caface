...
class BaseTrainer(LightningModule):
    ...
    def maybe_load_saved(self):
        if (
            not self.hparams.start_from_model_optim_statedict
            and not self.hparams.start_from_model_statedict
        ):
            # no loading
            return None
        ...
        if checkpoint_path.startswith("experiments"):
            proj_root = os.getcwd().split(os.environ.get("PROJECT_NAME"))[
                0
    ...
    def gather_outputs_v2(self, outputs, unique_index_name="idx"):
        ...
        value_keys = [
            key for key in outputs_list[0].keys() if key != unique_index_name
        ]  # ex: ['aggregated_features', 'template_type', 'template_idx']
        gathered_tensor_dict = {}
        for value_key in value_keys:
            gathered_tensor = torch.cat(
                [out[value_key] for out in outputs_list], axis=0
            ).to("cpu")
            gathered_tensor_dict[value_key] = gathered_tensor

        gathered_idx = torch.cat(
            [out[unique_index_name] for out in outputs_list], axis=0
        ).to("cpu")
        unique_dict = {}
        ...
        for value_key in value_keys:
            all_gathered_outputs[value_key] = torch.stack(
                [unique_dict[key][value_key] for key in unique_keys], axis=0
            )
    ...
    def configure_optimizers(self):
        bn_weight_decay = (
            self.hparams.weight_decay if self.hparams.optimizer_type == "adamw" else 0.0
        )
        ...
        model_config = {
            "params": model_paras_wo_bn,
        }
        model_only_bn_config = {
            "params": model_paras_only_bn,
            "weight_decay": bn_weight_decay,
        }

        agg_params_hasdecay, agg_params_nodecay = self.set_weight_decay(self.aggregator)
        agg_config = {
            "params": agg_params_hasdecay,
        }
        agg_only_bn_config = {
            "params": agg_params_nodecay,
            "weight_decay": bn_weight_decay,
        }
    ...
    def parse_saved_feature(self, features):
        if features.shape[-1] == 25600:
            # shape: 32, 4, 25600
            folded_features, folded_intermediates = torch.split(
                torch.tensor(features), [512, 25088], dim=2
            )
            B, T, _ = folded_intermediates.shape
            folded_intermediates = folded_intermediates.view(B, T, 512, 7, 7)
            raveled_intermediate = rearrange(
                folded_intermediates, "b t c h w-> (b t) c h w"
            )
            ...
            folded_features, folded_intermediates = torch.split(
                torch.tensor(features), [512, tot_styledim * 2], dim=2
            )
            B, T, _ = folded_intermediates.shape
            folded_intermediates = folded_intermediates.view(B, T, tot_styledim, 2)
            folded_intermediates_split = torch.split(
                folded_intermediates, split_dims, dim=2
            )  # [[4, 128, 2], [4, 256, 2]] <- [4, 384, 2]

            # only select style vectors you want to use ( e.g 1,3,5,7 -> 3,5 )
            folded_intermediates_split = [
                folded_intermediates_split[style_index_dict[using_style]]
                for using_style in self.hparams.style_index
            ]

            raveled_intermediates = [
                rearrange(split, "b t c1 c2-> (b t) c1 c2")
                for split in folded_intermediates_split
            ]
    ...
    def feature_extract(self, clean_images, extra_images):
        ...
        if clean_images.ndim == 5:  # image data
            ...
            folded_embeddings, folded_norms, folded_intermediate = self.ravel_forward(
                clean_images, extra_images
            )
            ...
            folded_features1, raveled_intermediates1 = self.parse_saved_feature(
                clean_images
            )
            folded_features2, raveled_intermediates2 = self.parse_saved_feature(
                extra_images
            )

            raveled_compressed_intermediate1 = self.aggregator.compress_intermediate(
                raveled_intermediates1
            )
            raveled_compressed_intermediate2 = self.aggregator.compress_intermediate(
                raveled_intermediates2
            )

            folded_intermediate1 = rearrange(
                raveled_compressed_intermediate1, "(b t) c-> b t c", b=B
            )
            folded_intermediate2 = rearrange(
                raveled_compressed_intermediate2, "(b t) c-> b t c", b=B
            )

            folded_mag_embeddings = torch.cat(
                [folded_features1, folded_features2], dim=1
            )
            folded_intermediate = torch.cat(
                [folded_intermediate1, folded_intermediate2], dim=1
            )
        else:
    ...
    def ravel_forward(self, images, extra_images):
        ...
        raveled_images = rearrange(
            stacked_images, "b t c h w -> (b t) c h w"
        ).contiguous()
        if self.hparams.intermediate_type == "map":
            with torch.no_grad():
                raveled_embeddings, raveled_norms, raveled_intermediate = self.model(
                    raveled_images, return_intermediate=True
                )
            raveled_compressed_intermediate = self.aggregator.compress_intermediate(
                raveled_intermediate
            )
        elif self.hparams.intermediate_type == "style":
            with torch.no_grad():
                raveled_embeddings, raveled_norms, raveled_intermediate = self.model(
                    raveled_images, return_style=self.hparams.style_index
                )
            raveled_compressed_intermediate = self.aggregator.compress_intermediate(
                raveled_intermediate
            )
        ...
        folded_intermediate = rearrange(
            raveled_compressed_intermediate, "(b t) c -> b t c", t=T
        )
