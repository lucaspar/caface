...
def get_args():
    ...
    parent_parser.add_argument(
        "--train_data_path", type=str, default="faces_emore/imgs"
    )
    parent_parser.add_argument(
        "--ijb_meta_path", type=str, default="IJB/insightface_helper/ijb"
    )
    parent_parser.add_argument(
        "--ijb_aligned_imgs_path", type=str, default="IJB/aligned/IJBB"
    )
    ...
    parent_parser.add_argument(
        "--use_16bit", action="store_true", help="if true uses 16 bit precision"
    )
    parent_parser.add_argument(
        "--epochs",
        default=90,
    ...
    parent_parser.add_argument(
        "--lr_milestones", default="8,12,14", type=str, help="epochs for reducing LR"
    )
    parent_parser.add_argument(
        "--lr_gamma", default=0.1, type=float, help="multiply when reducing LR"
    )
    ...
    parent_parser.add_argument(
        "--fast_dev_run", dest="fast_dev_run", action="store_true"
    )
    parent_parser.add_argument(
        "--evaluate", action="store_true", help="use with start_from_model_statedict"
    )
    parent_parser.add_argument("--resume_from_checkpoint", type=str, default="")

    parent_parser.add_argument(
        "--start_from_model_optim_statedict", type=str, default=""
    )
    ...
    args.output_dir = os.path.join(
        project_root, "experiments", args.prefix + "_" + current_time
    )
...
def add_task_arguments(parser):
    ...
    parser.add_argument(
        "--gradient_clip_value", type=float, default=0.0
    )  # default is no clip
    ...
    parser.add_argument(
        "--intermediate_type", type=str, default="style"
    )  # style or map
    ...
    parser.add_argument(
        "--datafeed_scheme",
        type=str,
        default="dual_multi_v1",
        choices=["single", "dual_multi_v1"],
    )
    parser.add_argument(
        "--img_aug_scheme", type=str, default="v2", choices=["v2", "v3"]
    )
