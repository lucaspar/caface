...
from nets.transformer import tconfig, transformer_module
...
class ClusteringBlock(nn.Module):
    ...
    def forward(self, x, group_tokens, values=None, return_attn=False):
        ...
        x = self.norm_x(x)
        new_value, attn_dict, new_query = self.assign(
            group_tokens, x, value=values, return_attn=return_attn
        )
...
class ClusteringLayer(nn.Module):
    ...
    def forward(self, x, prev_group_token=None, values=None, return_attn=False):
        ...
        new_value, attn_dict, new_query = self.cluster_block(
            x, group_token, values=values, return_attn=return_attn
        )
...
class ClusterAggregateTransformer(nn.Module):
    ...
    def __init__(
        ...
        cluster_block = ClusteringBlock(
            dim=dim, value_dim=value_dim, norm_layer=norm_layer, assign_eps=0.0001
        )
        ...
        for _ in range(depths[0]):
            self.conf_mlp.append(
                transformer_module.MixerBlock(
                    tokens_mlp_dim=num_group_tokens[0],
        ...
        self.conf_mlp.append(
            transformer_module.Mlp(
                embed_dim + embed_dim, embed_dim + embed_dim, confidence_dim
            )
        )
    ...
    def forward(self, style_features, latent_code, return_attn=False, memory=None):
        ...
        cluster_feature, group_token, attn_dict, cluster_style = self.clustering_layer(
            style_features, group_token, values=latent_code, return_attn=return_attn
        )
        attn_dict_list.append(attn_dict)

        # if memery, incremental update
        if memory is not None:
            cluster_feature = self.add_memory(
                cluster_feature,
                attn_dict["attn"],
                memory["attn"],
                memory["cluster_features"],
            )
            cluster_style = self.add_memory(
                cluster_style,
                attn_dict["attn"],
                memory["attn"],
                memory["cluster_style"],
            )

        # aggregation (AGN)
        confidence_weight = self.conf_mlp(
            torch.cat([group_token, cluster_style], dim=-1)
        )
        ...
        if random.random() < self.random_group_discard and self.training:
            ...
            mask = torch.ones(
                (B, T, 1), dtype=group_token.dtype, device=group_token.device
            )
...
def incremental_mean(prev_mean, prev_count, new_val, new_val_weight=1):
    return ((prev_count * prev_mean) + (new_val_weight * new_val)) / torch.clip(
        prev_count + new_val_weight, 1e-6, torch.inf
    )
...
if __name__ == "__main__":
    ctran = ClusterAggregateTransformer(
        depths=[3, 3], num_group_tokens=[64, 0], key_query_combined_info_propagate=True
    )

    out = ctran(
        style_features=torch.randn(4, 100, 384),
