...
class StyleBasedAggregator(nn.Module):
    ...
    def __init__(
        ...
        if combine_norm_emb:
            ...
            self.norm_emb = transformer_module.SinusoidalEncoding(
                d_model=normemb_dim, clip_val=3, clip_multi=10
            )
        ...
        if input_dim == input_maker_outdim:
            ...
            self.input_maker = transformer_module.Mlp(
                input_dim, input_maker_outdim, input_maker_outdim
            )
    ...
    def forward(self, folded_features, intermediate, return_attn=True):
        ...
        result = self.fusion_net(
            inputs, latent_code=latent_code, return_attn=return_attn, memory=self.memory
        )
        ...
        if self.memory_count > -1 and self.use_memory:
            ...
            if self.training:
                new_att_sum = (
                    result["attn_dict_list"][0]["attn"].squeeze(1).sum(-1, keepdim=True)
                )
            ...
            if self.memory is not None:
                self.memory = {
                    "attn": new_att_sum + self.memory["attn"],
...
if __name__ == "__main__":
    ...
    compressed_intermediate = rearrange(
        compressed_intermediate, "(b t) c -> b t c", t=T
    )
