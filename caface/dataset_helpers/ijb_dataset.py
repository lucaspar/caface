...
def make_face_ijbb_dataset(ijb_root, val_data_path, num_sample_per_identity, **kwargs):
    ...
    landmark_list_path = os.path.join(
        "{}/{}/meta/{}_name_5pts_score.txt".format(
            ijb_root, dataset_name, dataset_name.lower()
        )
    )
    ...
    templates, medias = eval_helper_verification.read_template_media_list(
        os.path.join(
            ijb_root,
            "%s/meta" % dataset_name,
            "%s_face_tid_mid.txt" % dataset_name.lower(),
        )
    )
...
class IJBDataset(Dataset):
    ...
    def do_normalize_inputs(self, x):
        ...
        # 0-1 to -1-1
        x = torch.tensor(x)
        return (torch.flip(x, dims=[0]) - self.mean_.to(x.device)) / self.std_.to(
            x.device
        )
...
def ijbb_evaluation(
    data_root, ijb_root, dataset_name, image_features, best_fn, all_intermediate=None
):
    # read metadata
    templates, medias = eval_helper_verification.read_template_media_list(
        os.path.join(
    ...
    if len(templates) == len(image_features):
        ...
        template_norm_feats, unique_templates = (
            eval_helper_verification.image2template_feature_custom(
        ...
        score = eval_helper_verification.verification(
            template_norm_feats, unique_templates, p1, p2
        )
        verification_result_str, result_dict = eval_helper_verification.calc_tpr_fpr(
            score, label
        )
