...
def groupby_ops(
    value: torch.Tensor, labels: torch.LongTensor, op="sum"
    ...
    result = torch.zeros_like(unique_labels, dtype=value.dtype).scatter_add_(
        0, labels, value
    )
    if op == "mean":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="../pretrained_models/AdaFaceWebFace4M.ckpt",
    )
    ...
    name = "center_{}_{}.pth".format(
        os.path.basename(args.pretrained_model_path)
        .replace(".pth", "")
        .replace(".ckpt", ""),
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader):
            ...
            sum_embedding, new_tgt, labels_count = groupby_ops(
                embedding.detach().cpu(), tgt, op="sum"
            )
