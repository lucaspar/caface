...
def extract_and_save_feature(i, item, q_out, augmenter, backbone, image_root):
    ...
    feature, norm, intermediate = backbone(
        aug_samples.to("cuda:0"), return_style=[3, 5]
    )

    # reshape features
    mag_feature = feature * norm
    intermediates = torch.cat(
        intermediate, dim=1
    )  # [[16, 128, 2], [16, 256, 2]] -> [16, 384, 2]
    intermediates = intermediates.view(intermediates.shape[0], -1)  # 16, 768
    save_features = torch.cat(
        [mag_feature, intermediates], dim=1
    )  # 16, 512 + 768 (1280)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_root", type=str, default="./path_to/webface4m_subset_images"
    )
    parser.add_argument(
